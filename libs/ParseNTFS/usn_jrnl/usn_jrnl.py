...
class UsnJrnl():
    ...
    def grouped_by_entry(self):
        ...
        for record in self.records:
            if record.file_reference_mft_entry not in result.keys():
                result[record.file_reference_mft_entry] = {}
            if record.file_reference_sequence_number not in result[record.file_reference_mft_entry].keys():
                result[record.file_reference_mft_entry][record.file_reference_sequence_number] = []
            result[record.file_reference_mft_entry][record.file_reference_sequence_number].append(record)
        return result
